...
class InsumoHistorial extends Conectar
{
    public function get_historial_insumo($cInsumo)
    {
        ...
        $sql = "SELECT cHistorialInsumo,DATE_FORMAT(fechaCambio, '%m/%d/%Y') as fechaCambio, nombreInsumo, cantidad, costo, cInsumo FROM historialInsumo WHERE cInsumo = ? ORDER BY fechaCambio DESC LIMIT 3";
